...
const signUpHandle = async (req, res) => {
	...
	try {
		...
		return res.status(400).json({ message: 'User could not be created', error: err });
	}
};
...
const signInHandle = async (req, res) => {
	...
	try {
		...
		return res.status(400).json({ message: 'User could not login', error: err.message });
	}
};
...
async function thirdPartAuthenticatio(req, res) {
	...
	try {
		...
		if (!exitingUser) {
			...
			const token = jwt.sign({ id: exitingUser._id }, env.JWT_SECRET_KEY, {
				expiresIn: 60000,
			});
		...
		return res.status(400).json({ message: 'User could not be created', data: err });
	}
}
...
async function updateAvatarHandler(req, res) {
	...
	try {
		...
		return res.status(400).json({ message: 'User could not been updated', data: e });
	}
}
...
async function allArtistsHandler(req, res) {
	try {
		...
		return res.status(400).json({ message: 'Artists not could been found', data: e });
	}
}
