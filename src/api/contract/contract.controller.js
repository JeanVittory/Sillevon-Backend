...
const { transporter, contractConfirmation, contractAlert } = require('../../utils/mailer');
...
async function createContractHandler(req, res) {
	...
	try {
		...
		return res.status(201).json({ message: 'Contracts created', data: contracts });
	} catch (e) {
		return res.status(400).json({ message: 'Contracts could not been created', data: e });
	}
}
...
async function updateContractHandler(req, res) {
	...
	try {
		...
		return res.status(200).json({ message: 'Contracts updated', data: newContracts });
	} catch (e) {
		return res.status(400).json({ message: 'Contracts could not been updated', data: e });
	}
}
...
async function getContractHandler(req, res) {
	try {
		...
		return res.status(400).json({ message: 'Contracts could not been found', data: e });
	}
}
...
async function acceptContractHandler(req, res) {
	...
	try {
		...
		return res.status(200).json({ message: 'Contracts updated', data: contract });
	} catch (e) {
		return res.status(400).json({ message: 'Contract could not been updated', data: e });
	}
}
...
async function deleteContractHandler(req, res) {
	...
	try {
		...
		return res.status(400).json({ message: 'Contract could not been deleted', data: e });
	}
}
